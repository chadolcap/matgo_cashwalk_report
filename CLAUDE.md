...
main.py
  ...
  LaunchXshellAndRunScript()
    ...
    _FillFileDialog()       ← WM_SETTEXT + WM_COMMAND(IDOK)
...
```
##HEADER## 2026-05-28 08-00  |  Matgo Game Server Status
================================================================
...
```powershell
# 태스크 이름 확인
Get-ScheduledTask | Where-Object { $_.TaskName -like "*Matgo*" -or $_.TaskName -like "*matgo*" }

# 일시 비활성화 (트리거·설정 유지)
Disable-ScheduledTask -TaskName "MatgoServerMonitor"

# 다시 활성화
Enable-ScheduledTask -TaskName "MatgoServerMonitor"
```
...
3. 우측 목록에서 `MatgoServerMonitor` 찾기
...
```powershell
Unregister-ScheduledTask -TaskName "MatgoServerMonitor" -Confirm:$false
```

재등록이 필요할 때는 이전 등록 절차를 반복한다 (작업 스케줄러 GUI 또는 `Register-ScheduledTask` PowerShell 명령).
